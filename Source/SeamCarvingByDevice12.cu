...
__global__ void findEnergyKernel_v2(uint8_t * inPixels, int width, int height, int filterWidth, 
                    int * outPixels)
{
    ...
    { 
        ...
        for (int filterR = 0; filterR < filterWidth; filterR++)
        {
            for (int filterC = 0; filterC < filterWidth; filterC++)
            {
                ...
                inPixelsC = min(max(0, inPixelsC), width - 1);
                uint8_t inPixel = inPixels[inPixelsR * width + inPixelsC];
                outPixelX += inPixel * dc_filter_X[filterR * filterWidth + filterC];
                outPixelY += inPixel * dc_filter_Y[filterR * filterWidth + filterC];
            }
        }
        outPixels[outPixelsR * width + outPixelsC] = abs(outPixelX) + abs(outPixelY);
    }
                        ...
                        uchar3 * outPixels, dim3 blockSize, int version)
{
    ...
    for (int i = 0; i < width - newWidth; i++)
    {
        ...
        findEnergyKernel_v2<<<gridSize, blockSize>>>(d_grayImg, currentWidth, height, filterWidth, d_energy);
        cudaDeviceSynchronize();
