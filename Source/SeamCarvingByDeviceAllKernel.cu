                    ...
                    uchar3 * outPixels)
{
	  ...
	  for (int i = 0; i < width - newWidth; i++)
    {
        ...
        if(i==0){
          writePnm((int*)grayImg,1,currentWidth, height, (char*)"1234.pnm");
          writePnm(srcInPixels,currentWidth, height, (char*)"12345.pnm");
        }
...
__global__ void findEnergyKernel_v1(uint8_t * inPixels, int width, int height, 
                    float * filterX, float * filterY, int filterWidth, 
        ...
        for (int filterR = 0; filterR < filterWidth; filterR++)
        {
            for (int filterC = 0; filterC < filterWidth; filterC++)
            {
                ...
                if (inPixelsR >= 0 && inPixelsR <= height - 1)
                    ...
                    {
                        float filterVal_X = filterX[filterR*filterWidth + filterC];
                        float filterVal_Y = filterY[filterR*filterWidth + filterC];
                        ...
                        uchar3 * outPixels, dim3 blockSize, int version)
{
    ...
    for (int i = 0; i < width - newWidth; i++)
    {
        ...
        findEnergyKernel_v1<<<gridSize, blockSize, w*h*sizeof(uint8_t)>>>(d_grayImg, currentWidth, height, d_filterX, d_filterY, filterWidth, d_energy, w, h);
        cudaDeviceSynchronize();
        CHECK(cudaGetLastError());
