...
__global__ void SeparableConvRow(uint8_t * inPixels, int width, int height, float * filter_X, float * filter_Y, int filterWidth,
                              int * tmp_arr_X, int * tmp_arr_Y)
{
    int r = blockIdx.y * blockDim.y + threadIdx.y;
    int c = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (r < height && c < width)
    {
        ...
        int start_column = c - filterWidth/2;
        int start_row = r;
        int outPixel_X = 0;
        int outPixel_Y = 0;
        ...
        for (int filterC = 0; filterC < filterWidth; filterC++)
        {
            int inPixelsC = start_column + filterC;
            int inPixelsR = start_row + filterR;
            
            if (inPixelsC >= 0 && inPixelsC <= width - 1)
            {
            	float filterVal_X = filter_X[filterR*filterWidth + filterC];
	            float filterVal_Y = filter_Y[filterR*filterWidth + filterC];
	            ...
	            tmp_arr_X[r*width + c] = outPixel_X;
	            tmp_arr_Y[r*width + c] = outPixel_Y;
            }	            
        }
...
__global__ void SeparableConvCol(int * tmp_arr_X, int * tmp_arr_Y, int width, int height, float * filter_X, float * filter_Y, int filterWidth,
                                 int * outPixels)
{
    int r = blockIdx.y * blockDim.y + threadIdx.y;
    int c = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    {
        int start_column = c;
        int start_row = r - filterWidth/2;
        int outPixel_X = 0;
        int outPixel_Y = 0;

        for (int filterR = 0; filterR < filterWidth; filterR++)
        {
            int inPixelsC = start_column;
            int inPixelsR = start_row + filterR;

            if (inPixelsR >= 0 && inPixelsR <= height - 1)
            {
	            float filterVal_X = filter_X[3 * filterR];
	            float filterVal_Y = filter_Y[3 * filterR];

	            int inPixel_X = tmp_arr_X[inPixelsR*width + inPixelsC];
	            int inPixel_Y = tmp_arr_Y[inPixelsR*width + inPixelsC];

	            outPixel_X += filterVal_X * inPixel_X;
	            outPixel_Y += filterVal_Y * inPixel_Y;
	        }
            ...
            int * energyMap, int * directions, int r)
{
      ...
      extern __shared__  int SEnergyMap[];
      SEnergyMap[threadIdx.x+1] =energyMap[(r + 1) * width + c];
      if(threadIdx.x==0&&c-1>=0){
        SEnergyMap[0]= energyMap[(r + 1) * width + c-1];
      }else
      if(threadIdx.x== blockDim.x-1 && c+1<width){
        SEnergyMap[blockDim.x+1]=  energyMap[(r + 1) * width + c+1];
      }
      __syncthreads();
      ...
      int leftC = max(c - 1, 0);
      int smemIdx=threadIdx.x;
      int rightC = min(c + 1, width - 1);
      if(c - 1 <0 ) smemIdx+=1;
      int min = SEnergyMap[smemIdx++];

      int nextIdx = leftC;
      for (int i = leftC + 1; i <= rightC; i++, smemIdx++)
      {
            int currentEnergy =SEnergyMap[smemIdx];
            if (currentEnergy < min)
            {
                min = currentEnergy;
                nextIdx = i;
                        ...
                        uchar3 * outPixels, dim3 blockSize, int version)
{
    ...
    for (int i = 0; i < width - newWidth; i++)
    {
        ...
        size_t pixelsSize = currentWidth * height * sizeof(int);
        int * tmp_arr_X, * tmp_arr_Y;
        CHECK(cudaMalloc(&tmp_arr_X, pixelsSize));
        CHECK(cudaMalloc(&tmp_arr_Y, pixelsSize));
        
        SeparableConvRow<<<gridSize, blockSize>>>(d_grayImg, currentWidth, height, d_filterX, d_filterY, filterWidth, tmp_arr_X, tmp_arr_Y);
        SeparableConvCol<<<gridSize, blockSize>>>(tmp_arr_X, tmp_arr_Y, currentWidth, height, d_filterX, d_filterY, filterWidth, d_energy);
        ...
        for (int r = height - 2; r >= 0; r--)
        {
            int smem= (blockSize.x+2)*sizeof(int);
            findEnergyMapAndDirectionsKernel<<<gridSize.x, blockSize.x,smem>>>(d_energy, currentWidth, height, d_energyMap, d_directions, r);
            cudaDeviceSynchronize();
            CHECK(cudaGetLastError());
        }
